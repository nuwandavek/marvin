...
class Model(object):
    ...
    def __init__(self, args, vocab):
        ...
        init_state = tf.concat([linear(labels, dim_y, scope='encoder'),
            tf.zeros([self.batch_size, dim_z])], 1) ##Changed self.batch_size
        ...
        teach_h = tf.compat.v1.concat([tf.expand_dims(self.h_ori, 1), g_outputs], 1)
        ...
        half = tf.compat.v1.to_int32(self.batch_size / 2)
...
def transfer(model, decoder, sess, args, vocab, data0, data1, out_path):
    ...
    print("Length of batches in Transfer method",len(batches))
...
if __name__ == '__main__':
    ...
    with tf.compat.v1.Session(config=config) as sess:
        ...
        if args.train:
            ...
            print("Total epochs", args.max_epochs)
            for epoch in range(1, 1+args.max_epochs): 
            #for epoch in range(1, 6): #FOR TESTING
                print('--------------------epoch %d--------------------' % epoch)
                print('learning_rate:', learning_rate, '  gamma:', gamma)
                print("Total Batches", len(batches))
                for i,batch in enumerate(batches):
                    #Break after 100 batches #FOR TESTING
                    #if(i==400): #FOR TESTING
                    #    break #FOR TESTING
                    print(i,batch['size'])
                ...
                if args.dev:
                    ...
                    #model.saver.save(sess, args.model) #FOR TESTING
