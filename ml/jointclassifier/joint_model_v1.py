...
class JointSeqClassifier(DistilBertPreTrainedModel):
    ...
    '''
    def __init__(self, config, tasks, model_args, task_if_single, joint, label_dims):
        ...
        self.classifiers = {}
        for task in tasks:
            self.classifiers[task] = nn.Linear(config.dim, label_dims[task])
        ...
        for t, task in enumerate(tasks):
            ...
            if labels != None:
                if self.label_dims[task] == 1:
                    ...
                    loss_dict[task] += loss_fct(logits.view(-1), labels.to(torch.float32).view(-1))
                else:
                    loss_fct = nn.CrossEntropyLoss()
                    loss_dict[task] += loss_fct(logits.view(-1, self.label_dims[task]), labels.view(-1))
    ...
    def predict(
        ...
        for t, task in enumerate(self.tasks):
            logits_dict[task] = self.classifier[task](pooled_output)  # (bs, num_labels)        
